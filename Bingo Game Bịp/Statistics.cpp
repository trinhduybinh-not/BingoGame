...
void Statistics::Reset() {
    ...
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 5; j++) {
            heatmap[i][j]    = 0;
            xTongPhieu[i][j] = 0;   // reset X stats
            xSoLanThang[i][j]= 0;
        }
...
void Statistics::record(const GameResult& result) {
    ...
    // ── Chỉ số 4: thống kê vị trí X của TẤT CẢ phiếu ──
    for (auto& vx : result.viTriXMoiPhieu) {
        if (vx.hang >= 0 && vx.hang < 5 && vx.cot >= 0 && vx.cot < 5) {
            xTongPhieu[vx.hang][vx.cot]++;
            if (vx.daThang) xSoLanThang[vx.hang][vx.cot]++;
        }
    }
...
void Statistics::XuatBCR(int soLuongTop) const {
    ...
    cout << "   BCR = lan tren duong thang / tong lan xuat hien\n";
    ...
    for (int t = 0; t < soLuongTop && t < 99; t++)
        cout << "  " << (t+1) << ". So " << setw(3) << idx[t]
             << " — BCR: " << fixed << setprecision(1) << bcr[idx[t]] << "%\n";
}

// ─────────────────────────────────────────────
//  Chỉ số 4: Thống kê vị trí X
//
//  TiLeThang[i][j] = xSoLanThang[i][j] / xTongPhieu[i][j]
//  Ô nào TiLeThang THẤP nhất → đặt X ở đó
//  người chơi khó thắng nhất → có lợi cho nhà cái
// ─────────────────────────────────────────────
void Statistics::XuatThongKeX() const {
    cout << "\n== CHI SO 4: THONG KE VI TRI O X ==\n";
    cout << "   Ti le thang = so phieu thang / tong phieu co X tai o do\n";
    cout << "   O X co ti le thang THAP NHAT = vi tri tot nhat cho nha cai\n\n";

    // Tính tỉ lệ thắng từng ô
    float tiLeThang[5][5] = {};
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 5; j++)
            if (xTongPhieu[i][j] > 0)
                tiLeThang[i][j] = (float)xSoLanThang[i][j] / xTongPhieu[i][j] * 100.0f;

    // Tìm ô thấp nhất và cao nhất
    int hangMin = 0, cotMin = 0, hangMax = 0, cotMax = 0;
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 5; j++) {
            if (tiLeThang[i][j] < tiLeThang[hangMin][cotMin])
                { hangMin = i; cotMin = j; }
            if (tiLeThang[i][j] > tiLeThang[hangMax][cotMax])
                { hangMax = i; cotMax = j; }
        }

    // In bảng tỉ lệ thắng
    cout << "  TI LE THANG (%) KHI X TAI O (hang, cot):\n";
    cout << "        [0]     [1]     [2]     [3]     [4]\n";
    for (int i = 0; i < 5; i++) {
        cout << "  [" << i << "]";
        for (int j = 0; j < 5; j++) {
            bool isMin = (i == hangMin && j == cotMin);
            bool isMax = (i == hangMax && j == cotMax);
            if (isMin)      cout << "\033[32m";  // xanh = thấp nhất (tốt cho nhà cái)
            else if (isMax) cout << "\033[31m";  // đỏ   = cao nhất (tệ cho nhà cái)
            cout << fixed << setprecision(1) << setw(7) << tiLeThang[i][j] << "%";
            if (isMin || isMax) cout << "\033[0m";
        }
        cout << "\n";
    }

    // In bảng số lượng phiếu
    cout << "\n  SO PHIEU CO X TAI O (hang, cot):\n";
    cout << "        [0]     [1]     [2]     [3]     [4]\n";
    for (int i = 0; i < 5; i++) {
        cout << "  [" << i << "]";
        for (int j = 0; j < 5; j++)
            cout << setw(8) << xTongPhieu[i][j];
        cout << "\n";
    }

    // Kết luận
    cout << "\n  KET LUAN:\n";
    cout << "\033[32m  => Vi tri TOT NHAT cho nha cai (ti le thang thap nhat):\n";
    cout << "     O (" << hangMin << ", " << cotMin << ")"
         << " — ti le thang: " << fixed << setprecision(2)
         << tiLeThang[hangMin][cotMin] << "%"
         << " (tren " << xTongPhieu[hangMin][cotMin] << " phieu)\n\033[0m";
    cout << "\033[31m  => Vi tri XAU NHAT cho nha cai (ti le thang cao nhat):\n";
    cout << "     O (" << hangMax << ", " << cotMax << ")"
         << " — ti le thang: " << tiLeThang[hangMax][cotMax] << "%"
         << " (tren " << xTongPhieu[hangMax][cotMax] << " phieu)\n\033[0m";
}
...
void Statistics::printFrequencyTable() const {
    ...
    inKeDuong('=', 60);
    cout << left << setw(6) << "  So" << setw(12) << "  Tan suat"
         << setw(10) << "  Ti le%" << "  Bieu do\n";
    inKeDuong('-', 60);
    for (auto& [num, cnt] : freq) {
        double ratio = tongSoVan > 0 ? 100.0 * cnt / tongSoVan : 0;
        cout << "  " << right << setw(3) << num
             << "  " << setw(9) << cnt
             << "  " << setw(5) << fixed << setprecision(1) << ratio << "%  ";
        inThanh(cnt, maxF, 18);
        cout << "\n";
    }
    inKeDuong('=', 60);
...
void Statistics::printFinancialReport() const {
    ...
    for (double p : loiNhuanMoiVan) { if (p >= 0) vanLoi++; else vanLo++; }
    ...
    cout << "  Giam thuong (x/k)  : " << prize.getDecayPercent()
         << "% moi " << prize.getDecayInterval() << " lan boc\n";
    inKeDuong('-', 55);
    cout << "  Tong thu           : " << setprecision(0) << tongThu << " dong\n";
    cout << "  Tong tra thuong    : " << tongThuong << " dong\n";
    cout << "  Tong loi nhuan     : ";
    if (tongLoiNhuan >= 0) cout << "\033[32m+" << tongLoiNhuan << " dong\033[0m\n";
    else                   cout << "\033[31m"  << tongLoiNhuan << " dong (LO!)\033[0m\n";
    cout << "  Ti suat loi nhuan  : ";
    if (tiSuat >= 0) cout << "\033[32m+" << setprecision(2) << tiSuat << "%\033[0m\n";
    else             cout << "\033[31m"  << tiSuat << "%\033[0m\n";
    inKeDuong('-', 55);
    cout << "  TB moi van — Thu   : " << setprecision(0) << avgThu    << " dong\n";
    cout << "  TB moi van — Thuong: " << avgThuong  << " dong\n";
    cout << "  TB moi van — Loi   : ";
    if (avgLoiNhuan >= 0) cout << "\033[32m+" << avgLoiNhuan << "\033[0m dong\n";
    else                  cout << "\033[31m"  << avgLoiNhuan << "\033[0m dong\n";
    cout << "  Loi nhuan cao nhat : \033[32m+" << loiNhuanMax << "\033[0m dong\n";
    cout << "  Loi nhuan thap nhat: ";
    if (loiNhuanMin >= 0) cout << "\033[32m+" << loiNhuanMin << "\033[0m dong\n";
    else                  cout << "\033[31m"  << loiNhuanMin << "\033[0m dong\n";
    inKeDuong('-', 55);
    ...
    inKeDuong('=', 55);
    if      (tiSuat > 10) cout << "  => Cau hinh rat CO LOI cho nha cai\n";
    else if (tiSuat > 0)  cout << "  => Nha cai co loi nhe\n";
    else                  cout << "  => Nha cai THUA LO!\n";
}

// ─────────────────────────────────────────────
//  Export CSV
// ─────────────────────────────────────────────
void Statistics::exportFrequencyCSV(const string& filename) const {
    ofstream f(filename);
    if (!f.is_open()) { cerr << "Khong mo duoc: " << filename << "\n"; return; }
    f << "so,soLanXuatHien\n";
    for (auto& [so, cnt] : freq) f << so << "," << cnt << "\n";
    f.close();
    cout << "  [1] Da xuat: " << filename << "\n";
}

void Statistics::exportProfitCSV(const string& filename) const {
    ofstream f(filename);
    if (!f.is_open()) { cerr << "Khong mo duoc: " << filename << "\n"; return; }
    f << "van,loiNhuan\n";
    for (int i = 0; i < (int)loiNhuanMoiVan.size(); i++)
        f << (i+1) << "," << fixed << setprecision(0) << loiNhuanMoiVan[i] << "\n";
    f.close();
    cout << "  [2] Da xuat: " << filename << "\n";
}
